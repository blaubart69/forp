...
namespace TestForp
{
	volatile LONG64 LineReaderCounter;

	TEST_CLASS(TestLineReader)
	{
	private:
		...
		void AssertReadline(LPCWSTR expectedLine)
		{
			...
			DWORD rc = reader->readline(&line, &cchLen);
			if (rc != 0)
			{
				WCHAR err[128];
		...
		{
			reader = new LineReader(hTmp->getReadHandle(), bufSize);
		}
	...
	public:
		...
		{
			InterlockedIncrement64(&LineReaderCounter);
			hTmp = new HelpTempFile((UINT)LineReaderCounter);
		}
